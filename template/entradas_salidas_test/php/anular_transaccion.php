...
$sql_maestro = "SELECT * FROM tran_transaccion WHERE tran_codigo = '" . $tcodigo . "'";
...
if ($item =  mysqli_fetch_assoc($r_maestro)) {
...
if($flujo > 0)
{
    while ($deta = mysqli_fetch_assoc($r_detalle)){
        ...
        VALUES('".strtoupper(trim($ttipo))."', '"."POR ANULACION: # $codigo ".trim($tconcepto)."', CURRENT_TIMESTAMP, $, '".$deta['trand_producto_codigo']."', '".trim($deta['trad_producto_nombre'])."', ".$deta['trand_unidad_codigo'].", '".trim($deta['trand_unidad'])."', ".$deta['trand_unidad_cantidad'] .", ".$deta['trand_cantidad'].");";
